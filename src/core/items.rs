...
fn read_item_config(config: &Value) -> Option<Arc<ItemConfig>> {
    ...
    // Read with `?` like every other flag, and deliberately: an items.json
    // regenerated by a generator that has lost these fields fails the parse and
    // panics at startup, instead of silently deciding that no item in the game
    // is a liquid -- which is the failure that made this feature necessary.
